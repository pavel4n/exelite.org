...
/**
 * A tool for comparing the blocks which are accepted/rejected by bitcoind/execoinj
 * It is designed to run as a testnet-in-a-box network between a single bitcoind node and execoinj
...
public class BitcoindComparisonTool {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (Rule rule : blockList.list) {
            if (rule instanceof BlockAndValidity) {
                ...
                if (!chain.getChainHead().getHeader().getHash().equals(bitcoindChainHead)) {
                    differingBlocks++;
                    log.error("bitcoind and execoinj acceptance differs on block \"" + block.ruleName + "\"");
                }
        ...
        log.info("Done testing.\n" +
                "Blocks which were not handled the same between bitcoind/execoinj: " + differingBlocks + "\n" +
